...
'''
def set_gpus():
    os.environ["CUDA_VISIBLE_DEVICES"] = str([0])[1:-1]
...
def collect_data():
    if os.path.isfile(''):
        ...
        files = [os.path.join('', f) for f in os.listdir('') if f[-4:] in ['.jpg', '.JPG', '.png', '.PNG']]
        return files
...
def preprocess_image(img):
    if img.shape[1] / img.shape[0] < 6.4:
        img = pad_image(img, (200, 31), 1)
    else:
        img = resize_image(img, (200, 31))
    if 1 == 1:
...
def predict_text(model, img):
    ...
    ctc_out = K.get_value(ctc_decode)[:, :16]
    result_str = ''.join(['0123456789'+string.ascii_lowercase+'-'[c] for c in ctc_out[0]])
